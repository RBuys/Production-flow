Attribute VB_Name = "PushReceived"
Option Explicit
...
Public Sub PushOrdersBackToJC()
    ...
    For r = 2 To lastRow
        ...
        reportValueB = Trim$(CStr(reportSheet.Cells(r, "B").Value))
        orderNumber = Trim$(CStr(reportSheet.Cells(r, "C").Value))
        requiredDate = reportSheet.Cells(r, "D").Value

        If orderNumber = "" Then GoTo NextRow
        If jobNumber = "" Or reportValueB = "" Then GoTo NextRow

        jcPath = FindJCFile(jobNumber)
        If jcPath = "" Then
            SavePendingPush jobNumber, reportValueB, orderNumber, _
                            requiredDate, "JC file not found"
            GoTo NextRow
        End If

        If WriteOrderToJC(jcPath, reportValueB, orderNumber, requiredDate) Then
            updatedCount = updatedCount + 1
            ClearPendingPush jobNumber, reportValueB
        Else
            SavePendingPush jobNumber, reportValueB, orderNumber, _
...
CleanExit:
    ...
    MsgBox updatedCount & " order number(s) sent back to JC files.", _
...
' ============================================================
' JC UPDATE (MATCH ON COLUMN E)
' ============================================================
Private Function WriteOrderToJC(ByVal jcPath As String, _
                                ByVal reportValueB As String, _
                                ByVal orderNumber As String, _
                                ByVal requiredDate As Variant) As Boolean
    Dim wb As Workbook
    Dim ws As Worksheet
    Dim r As Long
    Dim jcValueE As String
    ...
    For Each ws In wb.Worksheets
        For r = 9 To 38
            ...
            jcValueE = Trim$(CStr(ws.Cells(r, "E").Value))

            If jcValueE = reportValueB Then
                If Trim$(CStr(ws.Cells(r, "L").Value)) = "" Then

                    ws.Cells(r, "L").Value = orderNumber
